...
class AdvancedToolManager:
    ...
    async def register_tool(self, func: AsyncOrSyncToolFunction, require_confirmation: bool = False):
        """Register a function as a tool with confirmation tracking."""
        # Register with base manager (handles schema generation automatically)
    ...
    async def get_tools(self) -> List[Dict]:
        """Get all registered tools from base manager."""
    ...
    async def execute_tool_call(self, tool_call: ToolCall) -> Any:
        ...
        try:
            ...
            raise ValueError(f"Tool '{tool_call.name}' not found") from e
        except TypeError as e:
            raise ValueError(f"Invalid arguments for tool '{tool_call.name}': {e}") from e
        except Exception as e:
            raise RuntimeError(f"Tool execution failed for '{tool_call.name}': {e}") from e
...
class AdvancedChatContext:
    ...
    def update_system_prompt(self, prompt: str):
        """Update the system prompt efficiently without recreating context."""
        self.system_message = prompt
        self.base_context.set_system_prompt(prompt)
...
class AdvancedChatEngine:
    ...
    def set_system_prompt(self, prompt: str):
        ...
        self.context.update_system_prompt(prompt)
